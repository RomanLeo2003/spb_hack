...
class InferenceModel:
    def __init__(self, model_path='models/swingnet_1800.pth.tar', seq_len=64):
        '''
        :param model_path:
        :param seq_len:
        '''
    ...
    def inference(self, video_path='test_video.mp4'):
        print('Testing...')
        ...
        for sample in dl:
            ...
            batch = 0
            while batch * self.seq_len < images.shape[1]:
                if (batch + 1) * self.seq_len > images.shape[1]:
                    image_batch = images[:, batch * self.seq_len:, :, :, :]
                else:
                    image_batch = images[:, batch * self.seq_len:(batch + 1) * self.seq_len, :, :, :]
        ...
        return {'confidence': confidence, 'events': events}


    def show_events(self, confidence, events):
        # show images:
        for i, e in enumerate(events):
